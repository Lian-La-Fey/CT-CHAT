...
from llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_IMAGE_TOKEN
...
#             from llava.model.language_model.llava_llama import LlavaConfig
#             lora_cfg_pretrained = LlavaConfig.from_pretrained(model_path)
#             tokenizer = AutoTokenizer.from_pretrained(model_base, local_files_only=True,  use_fast=False)

#             print('Loading LLaVA from base model...')
#             lora_cfg_pretrained.pad_token_id = None
#             lora_cfg_pretrained.vocab_size = lora_cfg_pretrained.vocab_size - 1 - 4
#             model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True,local_files_only=True, config=lora_cfg_pretrained, **kwargs)

#             print(f"Adding pad token as '<pad>'")
#             # print(model)
#             # query_weights = model.model.mm_projector.attn_pool.query
#             # print("Shape:", query_weights.shape)
#             # print("Values (first few):", query_weights.view(-1)[:10])
...
#             print('Loading additional LLaVA weights...')
#             if os.path.exists(os.path.join(model_path, 'non_lora_trainables.bin')):
#                 print("os.path.exists(os.path.join(model_path, 'non_lora_trainables.bin'))")
#                 non_lora_trainables = torch.load(os.path.join(model_path, 'non_lora_trainables.bin'), map_location='cpu')
#             else:
#                 print("return torch.load(cache_file, map_location='cpu')")
#                 # this is probably from HF Hub
#                 from huggingface_hub import hf_hub_download
#                 def load_from_hf(repo_id, filename, subfolder=None):
#                     cache_file = hf_hub_download(
...
#             non_lora_trainables = {(k[11:] if k.startswith('base_model.') else k): v for k, v in non_lora_trainables.items()}
#             if any(k.startswith('model.model.') for k in non_lora_trainables):
#                 print("if any(k.startswith('model.model.') for k in non_lora_trainables):")
#                 non_lora_trainables = {(k[6:] if k.startswith('model.') else k): v for k, v in non_lora_trainables.items()}
#             model.load_state_dict(non_lora_trainables, strict=False)
